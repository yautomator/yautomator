...
import type { ApplicationQuestion } from '$lib/utils/common';
...
export const actions = {
    ...
    await Promise.all(
      answers.map(async ([questionId, answer]) => {
        const question = organization.applicationQuestions.find((q: ApplicationQuestion) => q._id === questionId);
  ...
  application: async ({ params }: RequestEvent) => {
    ...
    const response = await fetch(`${env.BASE_API_URL}/startup/${startupId}/organizations/${orgId}/ai`, {
      method: 'POST',
    });
