...
function _page($$payload) {
  const { data } = page;
  const each_array = ensure_array_like(data.organizations.filter((organization) => organization.organizationType === "ACCELERATOR"));
  $$payload.out += `<header class="svelte-1r3q028">`;
  {
    let base = function($$payload2) {
      $$payload2.out += `<!---->Organizations`;
    }, current = function($$payload2) {
      $$payload2.out += `<span>Accelerators</span>`;
    };
    Breadcrumb($$payload, {
      base,
      current
